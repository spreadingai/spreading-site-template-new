...
如果用户加入房间后，想要了解此房间的成员构成，可以调用 {getPlatformData(props,queryRoomMemberListMap)} 接口，传入参数 roomID，config，然后通过 [ZIMRoomMemberQueriedCallback](https://doc-zh.zego.im/article/api?doc=zim_API~java_android~interface~ZIMRoomMemberQueriedCallback) 中的 [onRoomMemberQueried](https://doc-zh.zego.im/article/api?doc=zim_API~java_android~interface~ZIMRoomMemberQueriedCallback#on-room-member-queried) 回调接口，返回房间内成员列表。
:::
:::if{props.platform="U3d"}
如果用户加入房间后，想要了解此房间的成员构成，可以调用 [QueryRoomMemberList](https://doc-zh.zego.im/article/api?doc=zim_API~java_android~class~ZIM#query-room-member-list) 接口，传入参数 roomID，config，然后通过 [ZIMRoomMemberQueriedCallback](https://doc-zh.zego.im/) 回调接口，返回房间内成员列表。
:::
...
zim_->queryRoomMemberList(string_room_id, config, [=](/zim-android/guides/room/const-std::vector<zimuserinfo>&-member_list,-const-std::string&-next_flag,-zim::zimerror-error_info) {
...
:::if{props.platform="window"}
```cpp
zim_->queryRoomOnlineMemberCount(string_room_id, [=](/zim-android/guides/room/unsigned-int-count,-zim::zimerror-error_info) {
    if (error_info.code != 0)
    {
