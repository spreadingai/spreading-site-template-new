...
用户通过调用 [sendExtendedRequest](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~class~IZegoCopyrightedMusic#send-extended-request) 接口，传入对应的 command、params，可以获取标签列表、搜索歌曲、榜单歌曲、歌词分片等信息，对应结果将通过 [ZegoCopyrightedMusicSendExtendedRequestCallback](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~interface~ZegoExpressDefines#zego-copyrighted-music-send-extended-request-callback) 回调获取。
...
m_pMusic->sendExtendedRequest(command, params, [=](/online-ktv-windows/client-api/int-errorcode,-std::string-command,-std::string-result){
...
| 参数 | 类型 | 描述 |
|------|------|------|
| group_id | String | 标签分组 ID。开发者可以通过 group_id，获取版权方 vendor_id 为 0（默认）、1、2 时的短分片高潮片段资源，详情请参考 [获取短分片高潮片段资源](/online-ktv-windows/client-api/apis-to-obtain-songs-and-lyrics)。 |
...
需通过 [获取标签列表](/online-ktv-windows/client-api/send-extended-request#获取标签列表) 中的标签 ID，可获取对应的歌曲 ID、歌曲名、歌曲专辑、以及版权相关信息等。
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ songs | Array of Object | 歌曲列表，详情可查看[songs](/online-ktv-windows/client-api/send-extended-request#songs)。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ songs | Array of Object | 歌曲列表，详情见[songs](/online-ktv-windows/client-api/send-extended-request#songs-2)。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ songs | Array of Object | 歌曲列表，详情可查看[songs](/online-ktv-windows/client-api/send-extended-request#songs-3)。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ songs | Array of Object | 歌曲列表，详情可查看[songs](/online-ktv-windows/client-api/send-extended-request#songs-4)。 |
##### songs
| 参数 | 类型 | 描述 |
|------|------|------|
| song_id | String | 歌曲 ID。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ songs | Array of Object | 歌曲列表，详情可查看[songs](/online-ktv-windows/client-api/send-extended-request#songs-5)。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ copyright | Array of JSON | 版权列表，详情见[copyright](/online-ktv-windows/client-api/send-extended-request#copyright)。 |
...
| └ subright | Object | 子版权，详情见[subright](/online-ktv-windows/client-api/send-extended-request#subright)。 |
...
| request_id | String | 请求 ID。 |
| data | Object | 响应数据。 |
| └ tags | Array of Object | 第 N 个人唱或多个人唱。，详情见[tags](/online-ktv-windows/client-api/send-extended-request#tags)。 |
