...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
| ------------------------------  | ---- | ----- |
| 组合消息  | 新增组合消息类型，支持在一条消息中包含多种内容项，包括文本、图片、音频、视频、文件以及自定义消息。可实现如文本+图片、文本+视频、文本+音频等多种组合方式，灵活满足不同场景需求。详情请参考 [收发消息 - 收发组合消息](/zim-ios/guides/messaging/send-and-receive-messages#收发组合消息)。| [ZIMMultipleMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMultipleMessage) |
| 图片格式  | 图片消息新增支持 HEIC 和 HEIF 格式，进一步扩展了兼容的图片类型，满足更多场景需求。 | [ZIMImageMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMImageMessage) |
| 图片宽高信息  | ZIMImageMessage 支持开发者在发送图片消息时传入原图、大图、缩略图等图片宽高信息。ZIMVideoMessage 支持开发者在发送视频消息时传入视频首帧图宽高信息。 | <ul><li>[ZIMImageMessage - originalImageSize](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMImageMessage#original-image-size)</li><li>[ZIMImageMessage - largeImageSize](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMImageMessage#large-image-size)</li><li>[ZIMImageMessage - thumbnailSize](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMImageMessage#thumbnail-size)</li><li>[ZIMVideoMessage - videoFirstFrameSize](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMVideoMessage#video-first-frame-size)</li></ul>  |

**改进优化**

| <div style={{width:"100px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 会话标记列表超限处理逻辑改进 | 当用户标记的会话数量达到上限（默认为 1000）时，ZIM 的处理逻辑从原先的报错优化为自动取消最早标记的会话，确保用户能够继续添加新的标记会话而不受限制。 | [setConversationMark](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-conversation-mark-mark-type-enable-conversation-infos-callback) |
| 单个会话的标记数量限制上调 | 每个会话可拥有的标记数量上限从 20 上调至 30。 | [setConversationMark](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-conversation-mark-mark-type-enable-conversation-infos-callback) |
| 群成员信息查询逻辑优化 | 群成员列表信息与用户数据库关联，保障获取本地最新的用户信息。| <ul><li>[queryGroupMemberListByGroupID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-group-member-list-by-group-id-config-callback)</li><li>[queryGroupMemberInfoByUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-group-member-info-by-user-id-group-id-callback)</li></ul> |

**接口变更**

<Note title="说明">接口变更详情请参考 [升级指南 - ZIM](/zim-ios/client-sdks/zim-upgrade-guide#2190-升级指南)。</Note>
...
| `sendMediaMessage` | 自 2.19.0 版本后，发送多媒体消息需使用 [sendMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback) 接口。`sendMediaMessage` 接口被废弃，以实现发送消息的统一性和便于后续的通用扩展。  |
...
| -----  | ---- | ----- |
| 群备注  | 支持用户对某个群组设置仅自己可见的群备注。详情请参考 [群资料管理 - 修改群备注](/zim-ios/guides/group/group-info)。| <ul><li>[updateGroupAlias](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#update-group-alias-group-alias-group-id-callback)</li><li>[groupAliasUpdated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-group-alias-updated-operated-user-id-group-id)</li></ul> |
| 用户在线状态 | 不仅支持用户通过订阅以持续关注其他用户的在线状态变化，还支持一次性查询其他用户的当下的在线状态，也支持用户查询自己的订阅列表。此外，在多端登录场景下，用户可以监听自己的在线平台变化。详情请参考 [用户状态订阅](/zim-ios/guides/users/online-status-subscription)。| <ul><li>[subscribeUsersStatus](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#subscribe-users-status-config-callback)</li><li>[userStatusUpdated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-user-status-updated)</li><li>[unsubscribeUsersStatus](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#unsubscribe-users-status-callback)</li><li>[queryUsersStatusByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-users-status-by-user-i-ds-callback)</li><li>[querySubscribedUserStatusListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-subscribed-user-status-list-with-config-callback)</li></ul> |
| 获取房间用户头像 | 用户现在可以通过 `ZIMRoomMemberInfo` 类的 `userAvatarUrl` 属性，查询房间内其他用户的头像信息。| [ZIMRoomMemberInfo > userAvatarUrl](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMRoomMemberInfo#user-avatar-url) |
| 切换房间  | 新增 `switchRoomFromRoomID ` 接口以切换房间，适用于秒开等需要平滑切换房间的业务场景。详情请参考 [房间管理 - 切换房间](/zim-ios/guides/room/manage-rooms) | [switchRoomFromRoomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#switch-room-from-room-id-from-room-id-to-room-info-is-create-when-room-not-existed-config-callback) |
...
| -----  | ---- | ----- |
| 首次拉取会话列表耗时优化 | 优化用户登录后首次拉取会话列表的时效，提升用户体验。 | [queryConversationListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-conversation-list-with-config-callback) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
| -----  | ---- | ----- |
| 内容审核 | 无需额外开发，即可实现对消息的安全审核。<ul><li>支持消息类型多样，涵盖文本消息、图片消息、语音消息和视频消息。</li><li>支持先审后发和先发后审两种审核方式。</li><li>支持根据不同会话类型的消息定制审核标签和尺度</li><li>支持针对审核结果配置不同的审核强度。</li><li>除了默认的敏感词汇外，您可以自行定义需要处理的词汇。</li></ul>详情请参考 [内容审核 - 能力概述](/zim-ios/guides/content-moderation/overview)。 | - |
| 会话标记 | 支持用户对会话设置标记，适用于需要关注某会话或无法处理某条会话的场景。可使用本功能实现会话分组，即对多个会话设置相同的标记。客户端实现流程请参考 [标记会话](/zim-ios/guides/conversation/mark-conversations)，服务端实现流程请参考 [设置会话标记](/zim-server/conversation/set-conversation-marks)。  | <ul><li>[setConversationMark](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-conversation-mark-mark-type-enable-conversation-infos-callback)</li><li>[设置会话标记](/zim-server/conversation/set-conversation-marks)</li></ul> |
| 会话列表查询 | 支持将标记、会话类型和是否包含未读消息作为过滤项获取会话列表。详情请参考 [标记会话](/zim-ios/guides/conversation/mark-conversations)。 | [queryConversationListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-conversation-list-with-config-callback-1) |
| 特定会话未读消息总数查询 | 支持按标记作为过滤项获取未读消息总数。详情请参考 [标记会话](/zim-ios/guides/conversation/mark-conversations)。 | [queryConversationTotalUnreadMessageCount](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-conversation-total-unread-message-count-with-config-callback) |
| 回复消息 | 支持用户在会话内针对某条消息进行回复，当前支持回复文本、图片、文件、音频、视频、合并和自定义消息。此外，还支持用户查询回复树，获取回复的完整消息列表。详情请参考 [回复消息](/zim-ios/guides/messaging/reply-to-a-message)。 | <ul><li>[replyMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#reply-message-message-to-original-message-config-notification-callback)</li><li>[queryMessageRepliedListByMessage](/zim-ios/client-sdks/querymessagerepliedlistbymessage)</li></ul> |
| 获取特定历史消息列表 | 支持查询指定的历史消息列表，适用于需要了解某条消息的上下文的场景。详情请参考 [获取历史消息](/zim-ios/guides/messaging/get-message-history)。 | [queryMessagesByMessageSeqs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-messages-by-message-seqs-message-seqs-conversation-id-conversation-type-callback) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 合并转发 | <ul><li>支持用户在收发消息后，通过构造合并消息体，传入发送消息接口即可实现转发合并消息；<br />说明：如需实现逐条转发，只需将已有消息作为参数传入发送消息接口即可。</li><li>支持查询合并消息包含的子消息具体内容。</li></ul> | <ul><li>[ZIMCombineMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMCombineMessage)</li><li>[sendMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-message-to-conversation-id-conversation-type-config-notification-callback)</li><li>[queryCombineMessageDetailByMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-combine-message-detail-by-message-message-callback)</li></ul> |
| 收发 @ 消息 | 支持用户在某个会话发送消息时，为消息设置提及指定用户（可以不在当前会话）或提及全部会话内成员。 | <ul><li>[ZIMMessage > mentionedUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage#mentioned-user-i-ds)</li><li>[ZIMMessage > isMentionAll](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage#is-mention-all)</li></ul> |
| 删除消息 | 支持用户一次性删除全部会话的所有消息。 | [deleteAllConversationMessagesWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#delete-all-conversation-messages-with-config-config-callback) |
| 保存会话草稿 | ZIM 支持用户退出单聊和群聊会话后仍在本地保存会话草稿，以便后续编辑。 | [setConversationDraft](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-conversation-draft-draft-conversation-id-conversation-type-callback) |
| 单聊会话免打扰 | 在原有“群聊会话免打扰”的基础上，新增支持对单聊会话设置免打扰。单聊会话接收新消息后，不推送通知。 | [setConversationNotificationStatus](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-conversation-notification-status-conversation-id-conversation-type-callback) |
| 禁止发言 | 支持群主对群组实现群组禁言和指定群成员禁言。群组禁言支持全体禁言、普通成员禁言、指定角色成员禁言。 | <ul><li>[muteGroup](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#mute-group-is-mute-group-id-config-callback)</li><li>[muteGroupMembers](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#mute-group-members-is-mute-user-i-ds-group-id-config-callback)</li></ul> |
| 好友管理 | 用户可以直接添加和删除好友、查看看好友列表、向用户发起好友申请、同意或拒绝好友申请、查看好友申请列表、检查其他用户与自己的好友关系、查询或修改好友信息，以及搜索好友。 | <ul><li>[addFriendByUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#add-friend-by-user-id-user-id-config-callback)</li><li>[deleteFriendsByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#delete-friends-by-user-i-ds-user-i-ds-config-callback)</li><li>[sendFriendApplicationToUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-friend-application-to-user-id-user-id-config-callback)</li><li>[acceptFriendApplicationFromUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#accept-friend-application-from-user-id-user-id-config-callback)</li><li>[rejectFriendApplicationFromUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#reject-friend-application-from-user-id-user-id-config-callback)</li><li>[checkFriendsRelationByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#check-friends-relation-by-user-i-ds-user-i-ds-config-callback)</li><li>[queryFriendListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-friend-list-with-config-config-callback)</li><li>[queryFriendApplicationListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-friend-application-list-with-config-config-callback)</li><li>[updateFriendAlias](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#update-friend-alias-friend-alias-user-id-callback)</li><li>[updateFriendAttributes](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#update-friend-attributes-friend-attributes-user-id-callback)</li><li>[queryFriendsInfoByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-friends-info-by-user-i-ds-user-i-ds-callback)</li><li>[searchLocalFriendsWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-local-friends-with-config-config-callback)</li></ul> |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 多端登录 | “多平台登录”策略支持配置互踢逻辑，当前仅支持 android、iOS 设备互踢，Windows、Mac 设备互踢。 | [loginWithUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) |
| 离线登录 | 支持用户在离线状态登录 IM 服务，访问本地 SDK 数据。 | [loginWithUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) |
| 黑名单管理 | 用户可以查询自己的黑名单、将指定用户拉黑（不再接收该用户消息）、移出黑名单、以及检查指定用户是否在黑名单内。 | <ul><li>[queryBlacklistWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-blacklist-with-config-config-callback)</li><li>[addUsersToBlacklistWithUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#add-users-to-blacklist-with-user-i-ds-user-i-ds-callback)</li><li>[removeUsersFromBlacklistWithUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#remove-users-from-blacklist-with-user-i-ds-user-i-ds-callback)</li><li>[checkUserIsInBlackListByUserID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#check-user-is-in-black-list-by-user-id-user-id-callback)</li></ul> |
| 插入本地消息 | 支持向房间会话插入本地消息。 | [insertMessageToLocalDB](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#insert-message-to-local-db-message-conversation-id-conversation-type-sender-user-id-callback) |
| 呼叫邀请 | 新增回调 [callInvitationCreated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-created-call-id)，呼叫邀请发起者可通过监听此回调，得知呼叫邀请已创建。 | [callInvitationCreated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-created-call-id) |
...
| <div style={{width:"130px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 优化 roomID 长度限制 | 支持最长 128 字节的 [roomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMRoomInfo#room-id)。| [ZIMRoomInfo > roomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMRoomInfo#room-id) |
| 用户信息相关类新增用户头像字段 | [ZIMUserInfo](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMUserInfo) 和 [ZIMGroupMemberInfo](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMGroupMemberInfo) 新增 `userAvatarUrl` 字段，用于设置或说明用户头像地址。<br />注意：暂不支持获取房间成员的用户头像字段，即通过 [queryRoomMemberListByRoomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-room-member-list-by-room-id-room-id-config-callback) 和 [queryRoomMembersByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-room-members-by-user-i-ds-user-i-ds-room-id-callback) 拿到的 `ZIMUserInfo` 的 `userAvatarUrl` 为空。 | <ul><li>[ZIMUserInfo](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMUserInfo)</li><li>[ZIMGroupMemberInfo](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMGroupMemberInfo)</li></ul> |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
|  呼叫邀请新增模式 | 新增进阶模式，支持用户在呼叫中邀请、退出呼叫和结束呼叫。 | <ul><li>[callingInviteWithInvitees](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#calling-invite-with-invitees-call-id-config-callback)</li><li>[callQuit](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#call-quit-config-callback)</li><li>[callEnd](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#call-end-config-callback)</li></ul> |
| 撤回他人消息 | 群组会话中，群主可以撤回他人发送的消息。 | [revokeMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#revoke-message-message-config-callback) |
...
| 设置消息拓展字段 | 新增仅本端可见的消息拓展字段，支持更新该字段，可用于展示消息翻译状态或其他内容。 | <ul><li>[localExtendedData](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage#local-extended-data)</li><li>[updateMessageLocalExtendedData](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#update-message-local-extended-data-local-extended-data-message-callback)</li></ul> | 
| 搜索本地消息 | 通过关键字、用户 ID 等条件对单个或所有 `单聊` 和 `群聊` 会话的本地消息进行搜索，获取符合条件的消息列表；也可以基于本地消息搜索会话。| <ul><li>[searchLocalMessagesByConversationID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-local-messages-by-conversation-id-conversation-id-conversation-type-config-callback)</li><li>[searchGlobalLocalMessagesWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-global-local-messages-with-config-config-callback)</li><li>[searchLocalConversationsWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-local-conversations-with-config-config-callback)</li></ul> |
| 搜索群组 | 支持基于关键字对群组名称进行搜索，同时支持将群成员名称和群成员昵称纳入搜索范围。 | [searchLocalGroupsWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-local-groups-with-config-config-callback) |
| 搜索群成员 | 支持基于关键字对指定群内的群成员名称进行搜索，同时支持将群成员昵称纳入搜索范围。 | [searchLocalGroupMembersByGroupID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#search-local-group-members-by-group-id-group-id-config-callback) |

**改进优化**

| <div style={{width:"130px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 呼叫邀请接收回调 | 优化呼叫邀请普通模式，支持在呼叫邀请的超时时间内，离线用户收到呼叫邀请在上线后能够立即被通知。| [callUserStateChanged](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-user-state-changed-call-id) |

**废弃删除**

| <div style={{width:"130px"}}>废弃接口</div> | 废弃说明 | 废弃版本 |
| -----  | ---- | ----- |
| <ul><li>[callInvitationAccepted](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-accepted-info-call-id)</li><li>[callInvitationRejected](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-rejected-call-id)</li><li>[callInviteesAnsweredTimeout](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitees-answered-timeout-call-id)</li></ul> | 为便利开发者监听呼叫邀请中用户的呼叫状态变化，新增 [callUserStateChanged](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-user-state-changed-call-id)，替代原呼叫邀请回调 `callInvitationAccepted`、`callInvitationRejected` 和 `callInviteesAnsweredTimeout`。 | 2.9.0 |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 置顶会话 | 用户可以选择想要优先查看的会话，将其固定在会话列表顶部。 | <ul><li>[updateConversationPinnedState](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#update-conversation-pinned-state-is-pinned-conversation-id-conversation-type-callback)</li><li>[queryConversationPinnedListWithConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-conversation-pinned-list-with-config-config-callback)</li></ul> |
| 自定义消息类型 | 新增自定义类型消息，开发者可自定义消息的类型，如投票类型、接龙类型、视频卡片类型等，并自行完成消息的解析。ZIM SDK 不负责定义和解析自定义消息的具体内容。 | [ZIMCustomMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMCustomMessage) |
| 查询会话信息 | 通过指定会话 ID 查询会话的详细信息。 | [queryConversation](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-conversation-conversation-id-conversation-type-callback) |
| 查询房间内用户状态 | 通过指定若干 userID 和 roomID，查询目标用户是否在指定房间内，从而进行业务逻辑设计，如邀请连麦。说明：最多支持一次性查询 10 名用户的信息。 | [queryRoomMembersByUserIDs](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-room-members-by-user-i-ds-user-i-ds-room-id-callback) |
...
| 消息体增加扩展字段 | [ZIMMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage) 增加 [extendedData](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage#extended-data) 字段，用户可以将用户头像和昵称等信息传入该字段，在聊天对话发送消息时实时展示。	| [extendedData](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessage#extended-data) |
| 新增发送消息状态回调 | 开发者可以通过监听此回调来完善消息发送状态的逻辑。根据消息状态的变更，开发者可以在 UI 上做相应提醒等。 | [messageSentStatusChanged](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-message-sent-status-changed) |
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
|消息撤回|  撤回发送成功的消息，默认撤回 2 分钟内的消息。撤回操作仅支持撤回单聊和群聊的消息，不支持撤回房间内消息。说明：如需配置撤回时间，请联系 ZEGO 技术支持。| <ul><li>[revokeMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#revoke-message-message-config-callback)</li><li>[ZIMMessageRevokedCallback](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-message-revoked-callback)</li></ul>|
|消息回执| [ZIMMessageSendConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessageSendConfig) 新增 hasReceipt 参数，支持在单聊和群聊会话中发送附带回执的消息。<br />ZIM SDK 支持在单聊和群聊会话查看其他用户是否阅读本端发送的信息，以及支持查看群聊中已读消息和未读消息的数量，以及对应的用户信息，**但是不支持信令消息、弹幕消息、以及房间内消息使用回执。**| <ul><li>[ZIMMessageSendConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMMessageSendConfig)</li><li>[sendMessageReceiptsRead](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-message-receipts-read-message-list-conversation-id-conversation-type-callback)</li><li>[sendConversationMessageReceiptRead](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-conversation-message-receipt-read-conversation-id-conversation-type-callback)</li><li>[queryGroupMessageReceiptReadMemberListByMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-group-message-receipt-read-member-list-by-message-message-group-id-config-callback)</li><li>[queryGroupMessageReceiptUnreadMemberListByMessage](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-group-message-receipt-unread-member-list-by-message-message-group-id-config-callback)</li><li>[queryMessageReceiptsInfoByMessageList](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-message-receipts-info-by-message-list-message-list-conversation-id-conversation-type-callback)</li></ul> |
| 呼叫邀请支持离线推送 | [ZIMCallInviteConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMCallInviteConfig) 新增 pushConfig 参数，支持向离线用户推送呼叫邀请。离线用户接受呼叫邀请需要搭配 ZPNs SDK 一起使用。 | [ZIMCallInviteConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMCallInviteConfig) |

**改进优化**

| <div style={{width:"130px"}}>优化项</div>  | 优化描述 | 相关接口 |
| -----  | ---- | ----- |
| 优化 Swift 语言 API 命名 | 针对 Swift，ZIM 对现有的 Objective-C 接口自动生成的 Swift API 做了手动校正，避免因 XCode 自动生成接口导致 API 歧义等问题。<br />说明：若开发者使用 Swift 集成 ZIM SDK 2.5.0 版本后，出现了接口编译失败问题，只需按照 IDE 指示更改即可。接口变更详情请参考 [升级指南 - ZIM](/zim-ios/client-sdks/zim-upgrade-guide)。 | -|
| [ZIMPushConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMPushConfig) 的额外字段命名变更| 为避免开发者接入概念混淆，[ZIMPushConfig](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMPushConfig) 的 extendedData 参数修改为 payload 参数，功能不变。开发者在使用 ZPNs SDK 时，通过厂商通道的 extras 里面的 payload 字段即可获取到发送端传入的额外字段，详情请参考 [自定义点击通知获取 payload 字段](/zim-android/offline-push-notifications/get-payload-field)。<p>说明：若集成 ZIM SDK 2.5.0 版本后，开发者使用了离线推送功能后出现了接口编译失败的问题，请将 extendedData 改为 payload。</p> | - |
...
从 2.5.0 版本开始，iOS SDK 不再支持 bitcode，请参考 [Xcode 14 Release Notes](https://developer.apple.com/documentation/xcode-release-notes/xcode-14-release-notes#Deprecations) 中关于废弃 bitcode 的说明。
...
<table>
  ...
  <tr>
    ...
    <td><ul><li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-peer-message-message-to-user-id-config-callback">sendPeerMessage</a></li><li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#send-room-message-message-to-room-id-config-callback">sendRoomMessage</a></li></ul></td>
  </tr>
  <tr>
    <td>部分回调接口，增加 roomID 参数，便于开发者在调用相关接口时，获取当前操作结果来源的房间 roomID。</td>
    <td><ul>
<li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-left-callback">ZIMRoomLeftCallback</a></li>
<li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-member-queried-callback">ZIMRoomMemberQueriedCallback</a></li>
<li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-online-member-count-queried-callback">ZIMRoomOnlineMemberCountQueriedCallback</a></li>
<li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-attributes-batch-operated-callback">ZIMRoomAttributesBatchOperatedCallback</a></li>
<li><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-attributes-queried-callback">ZIMRoomAttributesQueriedCallback</a></li>
</ul></td>
  </tr>
  <tr>
    <td><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-attributes-operated-callback">ZIMRoomAttributesOperatedCallback</a> 增加了 roomID 和 errorKeys 参数，便于开发者获取房间属性时，获取当前操作结果来源的房间 roomID，同时操作失败的 key 会返回给开发者。</td>
    <td><a target="_blank" href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~class~ZIMDefines#zim-room-attributes-operated-callback">ZIMRoomAttributesOperatedCallback</a></td>
  </tr>
</tbody></table>
...
| [deleteRoomAttributesByKeys](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#delete-room-attributes-by-keys-room-id-config-callback) | 删除房间属性。调用此接口，传入 Key 即可删除，支持同时删除多个 Key-Value 属性对。 |
| [queryRoomAllAttributesByRoomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#query-room-all-attributes-by-room-id-room-id-callback) | 查询房间内设置的属性信息。 |
| [beginRoomAttributesBatchOperationWithRoomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#begin-room-attributes-batch-operation-with-room-id-config) | <p>开始进行房间属性的组合式操作，支持组合调用设置和删除房间属性。组合操作可将同一房间中，多个不同操作合并为一个原子操作进行执行，通常用于连续操作时不想被其他用户操作插入执行的场景中。</p><p>在此接口之后调用的 [setRoomAttributes](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#set-room-attributes-room-attributes-room-id-config-callback) 和 [deleteRoomAttributesByKeys](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#delete-room-attributes-by-keys-room-id-config-callback) 将会被同时发送到后台执行。</p> |
| [endRoomAttributesBatchOperationWithRoomID](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#end-room-attributes-batch-operation-with-room-id-callback) | <p>结束进行房间属性的组合式操作。</p><p>调用此接口之后将请求后台执行组合式操作。</p> |
| [roomAttributesUpdated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-attributes-updated-room-id) | 房间内有属性对更新时，相关信息的回调。 |
| [roomAttributesBatchUpdated](https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-attributes-batch-updated-room-id-update-info) | 房间内有使用组合式操作来更新房间属性时，相关信息的回调。|
...
| <div style={{width:"130px"}}>功能项</div>  | 功能描述 | 相关接口 |
|-------|-------|-------|
| 支持 SDK 线上依赖 | 支持开发者使用 JitPack 自动集成 SDK，提升集成效率。 | [实现基本消息收发 - 集成](/zim-ios/send-and-receive-messages) |
