...
```Cpp
// 定义回调
auto callinvite_callback = [=](/zim-macos/guides/std::string-callid,-zim::zimcallinvitationsentinfo-info,-zim::zimerror-errorinfo) {
    if (errorInfo.code != 0) {
...
:::if{props.platform="window"}
```Cpp
auto callcancel_callback = [=](/zim-macos/guides/std::vector<std::string>-errorinvitees,-zim::zimerror-errorinfo) {
    if (errorInfo.code != 0) {
...
auto callaccept_callback = [=](/zim-macos/guides/zim::zimerror-errorinfo) {
    if (errorInfo.code != 0) {
...
```Cpp
// 拒绝呼叫邀请
auto callreject_callback = [=](/zim-macos/guides/zim::zimerror-errorinfo) {
    if (errorInfo.code != 0) {
...
ZIM::getInstance()->callJoin("callID", config, [=](/zim-macos/guides/const-std::string-&callid,-const-zimcalljoinsentinfo-&info,-const-zimerror-&errorinfo){
    if(errorInfo.code == ZIM_ERROR_CODE_SUCCESS){
        // 加入成功后的业务逻辑
    }else{
        // 根据官网错误码表处理错误
