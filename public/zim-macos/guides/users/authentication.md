...
export const renewTokenMap = {
  'Android': <a href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIM#renew-token-callback" target="_blank">renewToken</a>,
  'iOS': <a href="https://doc-zh.zego.im/article/api?doc=zim_API~objective-c_macos~protocol~ZIM#renew-token-callback" target="_blank">renewToken</a>,
...
zim->login(userID, config, [=](/zim-macos/guides/users/zim::zimerror-errorinfo) {
...
void onTokenWillExpire(ZIM * zim, unsigned int second) override {
    ...
    zim->renewToken(token, [=](/zim-macos/guides/users/const-std::string-&token,-zim::zimerror-errorinfo) {
        // 开发者可根据 ZIMError errorInfo 判断房间登录是否成功。
        ......
    });
}
