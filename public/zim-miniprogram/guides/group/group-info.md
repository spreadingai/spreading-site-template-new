...
- 用户使用“群资料管理”功能之前，请先加入某个群组，否则无法使用相关功能，详情请参考 [加入群组](/zim-miniprogram/guides/group/manage-groups#加入群组)。
...
zim_->queryGroupInfo(group_id, 
    [=](/zim-miniprogram/guides/group/const-zimgroupfullinfo-&groupinfo,-zim::zimerror-errorinfo) {
...
zim_->updateGroupName("new_group_name", group_id, 
    [=](/zim-miniprogram/guides/group/const-std::string-&groupid,-const-std::string-&groupname,-zim::zimerror-errorinfo){
...
zim_->updateGroupNotice("new_group_notice", group_id, 
    [=](/zim-miniprogram/guides/group/const-std::string-&groupid,-const-std::string-&groupnotice,-zimerror-errorinfo){
        int error_code = errorInfo.code;
    });
```
</CodeGroup>
...
zim_->updateGroupAlias(group_alias, group_id, 
    [=](/zim-miniprogram/guides/group/const-std::string-&groupid,-const-std::string-&groupalias,-zim::zimerror-errorinfo){
        int error_code = errorInfo.code;    
    });
```
:::
...
此外，该用户还会收到 {getPlatformData(props,onConversationChangedMap)} 回调，得知 `conversationAlias`（会话别名）更新，详情请参考 [获取会话列表 - 监听会话变更](/zim-miniprogram/guides/conversation/get-the-conversation-list)。
