...
1. **发送广播消息**
    ...
    通过 [ZegoIMSendBroadcastMessageCallback ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressDefines#zego-im-send-broadcast-message-callback) 回调获取消息发送结果。
...
1. **发送弹幕消息**
    ...
    通过 [ZegoIMSendBarrageMessageCallback ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressDefines#zego-im-send-barrage-message-callback) 回调获取消息发送结果。
...
1. **发送自定义信令**
    ...
    调用 [sendCustomCommand ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#send-custom-command-callback) 接口向同一房间内通过 “toUserList” 指定的用户发送自定义信令，长度不能超过 1024 字节。

    通过 [ZegoIMSendCustomCommandCallback ](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressDefines#zego-im-send-custom-command-callback) 回调获取消息发送结果。
