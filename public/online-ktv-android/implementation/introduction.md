...
独唱方案，用户上麦后自由点歌后进行排麦演唱。演唱者进行歌曲演唱时，将麦克风人声和媒体播放器声音进行混流，观众在台下接收音频流。具体实现请查看 [独唱方案](/online-ktv-android/implementation/ktv-solutions/solo)。
:::
...
#### 实时方案
:::if{props.platform=undefined}
在歌房中，麦上用户本地同时播放伴奏，同时开唱。其他用户可以中途加入合唱。低延迟实时合唱体验，真正实时合唱。服务器对人声和伴奏精准对齐，保证观众收听体验。具体实现请查看 [实时合唱方案](/online-ktv-android/implementation/ktv-solutions/chorus/concurrent)。
:::
:::if{props.platform="Windows"}
...
#### 串行方案
:::if{props.platform=undefined}
合唱各方串行加入，主唱推出一条流，包含了伴奏和人声，副唱跟着主唱的伴奏进行合唱，观众再拉由副唱发出的混流。具体实现请查看 [串行合唱方案](/online-ktv-android/implementation/ktv-solutions/chorus/serial)。
:::
:::if{props.platform="Windows"}
...
### 抢唱
:::if{props.platform=undefined}
抢唱，是一种兼具“唱歌+社交+游戏竞技”属性的K歌玩法，在一轮抢唱游戏中，会播放若干首歌曲的高潮片段供玩家抢麦，玩家获得麦权可开始演唱歌曲。具体实现请查看 [抢唱方案](/online-ktv-android/implementation/ktv-solutions/grab-the-mic)。
:::
:::if{props.platform="Windows"}
...
可以参考 [点歌管理](/online-ktv-android/implementation/song-selection) 实现排麦和已点列表功能。

#### 正版曲库

ZEGO 正版曲库支持歌单展示、搜索歌曲、点歌，逐字歌词下载等。更多具体的内容查看 [ZEGO 内容中心 - 点歌](/online-ktv-android/zego-content-center/sing-songs)。
...
更多具体的内容查看 [评分功能](/online-ktv-android/implementation/vocal-scoring)。
