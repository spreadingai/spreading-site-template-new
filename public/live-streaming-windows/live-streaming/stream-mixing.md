...
可参考 [快速开始 - 实现流程](https://doc-zh.zego.im/article/13416#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
...
```cpp
engine->startMixerTask(task, [=](/live-streaming-windows/live-streaming/zegomixerstartcallback-result){
...
```cpp
// 传入之前创建的混流任务对象
engine->stopMixerTask(task, [=](/live-streaming-windows/live-streaming/int-errorcode){
    if (errorCode == 0) {
...
请参考 [快速开始 - 实现流程](https://doc-zh.zego.im/article/13416#CreateEngine) 的 “创建引擎” 和 “登录房间” 完成房间登录。
...
通过 [mixMode](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~enum~ZegoAudioMixMode) 参数可以修改多路音频流混音模式，目前支持如下多路音频流混音模式:
...
```cpp
engine->startAutoMixerTask(task,[=](/live-streaming-windows/live-streaming/zegomixerstartcallback-result){
    if(result.errorCode == 0)
    {
...
```cpp
// 传入之前创建的混流任务对象
engine->stopAutoMixerTask(task, [=](/live-streaming-windows/live-streaming/int-errorcode){
    if (errorCode == 0) {
...
3. **混流输入对象 [ZegoMixerInput](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~struct~ZegoMixerInput) 的 “ZegoRect” 布局的比例与这条流本身的分辨率不相符时，画面将如何裁剪？**
    ...
    SDK 会做等比缩放。假设一条输入流的分辨率为 “720 × 1280”，即比例为 “9:16”，同时这条流的 [ZegoMixerInput](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~cpp_windows~struct~ZegoMixerInput) 的 “layout” 参数为 “ZegoRect(0, 0, 100, 100)”，即比例为 “1:1” 时，画面将会显示这条流的中间部分，即上下部分被裁剪掉。
