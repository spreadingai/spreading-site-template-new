...
调用 [setCloudProxyConfig](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#set-cloud-proxy-config) 配置云代理。

<Warning title="注意">


此接口必须在调用 [createEngine](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#create-engine) 接口创建引擎前调用。
</Warning>
...
<Accordion title="设置云代理不生效要如何解决？" defaultOpen="false">
  ...
  - 确保在 [createEngine](https://doc-zh.zego.im/article/api?doc=Express_Audio_SDK_API~cpp_linux~class~ZegoExpressSDK#create-engine) 前正确设置云代理，不支持中途修改。
</Accordion>
