...
<td rowspan="8">
消息可靠有序，可存储为历史消息（保存时间请参考 [计费说明 - 版本差异](/zim-uniapp-x/introduction/pricing) 中“历史消息存储天数”）。<br />
...
- 已获取登录 SDK 所需的 Token，详情请参考 [使用 Token 鉴权](/zim-uniapp-x/guides/users/authentication)。
...
1. 请参考 [下载](/zim-uniapp-x/client-sdks/sdk-downloads) 页面，获取最新版本的 SDK 到本地，并将得到的 “zego-zim-uts.zip” 文件解压缩。
...
请参考 [跑通示例源码](/zim-uniapp-x/sample-code) 获取源码。
...
详细的接口介绍，请参考 [onConnectionStateChanged](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#connection-state-changed)、[onPeerMessageReceived](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#peer-message-received)、[onTokenWillExpire](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#token-will-expire)。
...
- 如果您使用的是 “Token 鉴权”，请参考 [使用 Token 鉴权](/zim-uniapp-x/guides/users/authentication) 文档，获取 Token 后，并在登录 ZIM 时传入 Token，鉴权通过后才能登录成功。
</Warning>
...
客户端 B 登录 ZIM 后，通过 [onPeerMessageReceived](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#peer-message-received) 监听接口，收到客户端 A 发送过来的消息。
