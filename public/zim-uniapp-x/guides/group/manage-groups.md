...
群组管理功能可应用于办公群、社交群、兴趣群以及粉丝群等场景中，群组成员数量上限请参考 [计费说明](/zim-uniapp-x/guides/content-moderation/instruction)。
...
开发者可以通过 {getPlatformData(props,ZIMGroupCreatedCallbackMap)} ，判断群组是否创建成功。相关错误码请查看 [常见错误码](/zim-uniapp-x/sdk-error-codes/zim)。

<Warning title="注意">

- “groupID” 支持开发者自定义规则生成，仅支持数字，英文字符和 {"'!'、'#'、'
<Content platform="UTS" />、'%'、'&'、'('、')'、'+'、'-'、':'、';'、'<'、'='、'.'、'>'、'?'、'@'、'['、']'、'^'、'_'、'{'、'}'、'|'、'~'"}，且不能以 ’#‘ 开头；若该字段为空，ZIM 服务器会自动生成。建议开发者将 “groupID” 设置为一个有意义的值，可将其与自己的业务账号系统进行关联。
- 调用 {getPlatformData(props,createGroupMap)} 接口创建群组后，会直接加入群组，无需再调用 {getPlatformData(props,joinGroupMap)} 接口加入群组。
- 用户创建群组时，该用户即是该群组的“群主”。如果想要转让身份，请参考 [转让群主](/zim-uniapp-x/guides/group/group-members#转让群主)。
...
<Content platform="UTS" />, '%', '&', '(', ')', '+', '-', ':', ';', '<', '=', '.', '>', '?', '@', '[', ']', '^', '_', '{', '}', '|', '~'，且不能以 ’#‘ 开头。
...
zim_->createGroup(group_info, user_list, config, 
    [=](/zim-uniapp-x/guides/group/const-zim::zimgroupfullinfo-&groupinfo,-const-std::vector<zim::zimgroupmemberinfo>-&userlist,-const-std::vector<zim::zimerroruser>-&erroruserlist,-zim::zimerror-errorinfo-{-int-error_code-=-errorinfo.code;-});
```
:::
...
zim_->joinGroup(group_id,
                [=](/zim-uniapp-x/guides/group/const-zim::zimgroupfullinfo-&groupinfo,-zim::zimerror-errorinfo) {
...
zim_->sendGroupJoinApplication(
    group_id, config,
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-const-zim::zimerror-&errorinfo) {
        int error_code = errorInfo.code;
    });
```
:::
...
zim_->acceptGroupJoinApplication(
    ...
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-const-std::string-&userid,-const-zim::zimerror-&errorinfo) {
        int error_code = errorInfo.code;
    });
```
:::
...
zim_->sendGroupInviteApplications(
    ...
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-const-std::vector<zim::zimerroruserinfo>-&erroruserlist,-const-zim::zimerror-&errorinfo) {
        int error_code = errorInfo.code;
    });
```
:::
...
zim_->acceptGroupInviteApplication(
    user_id, group_id, config,
    [=](/zim-uniapp-x/guides/group/const-zim::zimgroupfullinfo-&group,-const-std::string-&inviteruserid,-const-zim::zimerror-&errorinfo) {
        int error_code = errorInfo.code;
    });
```
:::
...
zim_->rejectGroupInviteApplication(
      user_id, group_id, config,
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-const-std::string-&inviteruserid,-const-zim::zimerror-&errorinfo) {
        int error_code = errorInfo.code;
    });
```
:::
...
```cpp
// 主动退出群组
zim_->leaveGroup(group_id, [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-zim::zimerror-errorinfo){
...
zim_->kickGroupMembers(user_list, group_id, 
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-const-std::vector<std::string>-&kickedmemberids,-const-std::vector<zimerroruser>-&erroruserlist,-zim::zimerror-errorinfo) {
...
```cpp
// 群主解散群组
zim_->dismissGroup(group_id, [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-zim::zimerror-errorinfo) {
    int error_code = errorInfo.code;
});
```
:::
...
// 用户查询自己加入了哪些群组
zim_->queryGroupList(
    [=](/zim-uniapp-x/guides/group/const-std::vector<zimgroupinfo>-&grouplist,-zim::zimerror-errorinfo){
        int error_code = errorInfo.code;
    });
```
:::
...
zim_->searchLocalGroups(searchConfig,
                        [=](/zim-uniapp-x/guides/group/const-std::vector<zim::zimgroupsearchinfo>-&groupsearchinfolist,-unsigned-int-nextflag,-const-zim::zimerror-&errorinfo) {
                            // 开发者可从 groupSearchInfoList 中获取到群组信息
                        });
```
...
如果您希望禁止特定群组成员发言，请参考 [群成员管理 - 设置群成员禁言状态](/zim-uniapp-x/guides/group/group-members)。
</Note>
...
zim_->muteGroup(is_mute, group_id, config,
                    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-bool-is_muted,-const-zim::zimgroupmuteinfo-&info,-const-zim::zimerror-&errorinfo) {
...
- 参考 [会话管理 - 拉取会话列表](/zim-uniapp-x/guides/conversation/manage-unread-message-counts#拉取会话列表) 主动拉取会话列表。
- 参考 [会话管理 - 更新会话列表](/zim-uniapp-x/guides/conversation/manage-unread-message-counts#更新会话列表)，监听会话变更回调，更新会话列表。
...
zim_->updateGroupJoinMode(
    ...
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-zim::zimgroupjoinmode-mode,-const-zim::zimerror-&errorinfo) {
        // 业务代码
    });
...
zim_->updateGroupInviteMode(
    ...
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-zim::zimgroupinvitemode-mode,-const-zim::zimerror-&errorinfo) {
        // 业务代码
    });
...
zim_->updateGroupBeInviteMode(
    ...
    [=](/zim-uniapp-x/guides/group/const-std::string-&groupid,-zim::zimgroupbeinvitemode-mode,-const-zim::zimerror-&errorinfo) {
        // 业务代码
    });
```
:::
...
zim_->queryGroupApplicationList(
    config, [=](/zim-uniapp-x/guides/group/const-std::vector<zim::zimgroupapplicationinfo>-&applicationlist,-unsigned-long-long-nextflag,-const-zim::zimerror-&errorinfo) {
        // 业务代码
    });
```
:::
