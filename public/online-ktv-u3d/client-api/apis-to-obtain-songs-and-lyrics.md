...
其中，[获取歌曲伴奏资源](/online-ktv-u3d/client-api/apis-to-obtain-songs-and-lyrics#获取歌曲伴奏资源)、[获取长分片高潮片段资源](/online-ktv-u3d/client-api/apis-to-obtain-songs-and-lyrics#获取长分片高潮片段资源)、[获取短分片高潮片段资源](/online-ktv-u3d/client-api/apis-to-obtain-songs-and-lyrics#获取短分片高潮片段资源) 可获取的资源片段，区别如下：
...
m_pMusic->requestResource(config, resourceType, [=](/online-ktv-u3d/client-api/int-errorcode,-std::string-resource) {
    //resource: 回调结果
    });
```
:::
...
m_pMusic->getSharedResource(config, resourceType, [=](/online-ktv-u3d/client-api/int-errorcode,-std::string-resource) {
    //resource: 回调结果
    });
```
:::
...
m_pMusic->getLrcLyric(song_id, vendorID, [=](/online-ktv-u3d/client-api/int-errorcode,-std::string-lyrics) {
    //lyrics: 回调结果
    });
```
:::
...
```cpp
m_pMusic->getKrcLyricByToken(krc_token, [=](/online-ktv-u3d/client-api/int-errorcode,-std::string-lyrics) {
    //lyrics: 回调结果
    });
```
:::
