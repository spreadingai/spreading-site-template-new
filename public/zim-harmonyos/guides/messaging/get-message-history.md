...
- 开发者请根据业务需要，查看 [收发消息](/zim-harmonyos/guides/messaging/send-and-receive-messages)、[删除消息](/zim-harmonyos/guides/messaging/delete-messages) 等功能。
- 历史消息存储天数与套餐版本相关，详情请参考 [计费说明](/zim-harmonyos/introduction/pricing#版本差异) 的 “版本差异”。
</Note>
...
ZIM::getInstance()->queryMessages(messageSeqs, conversationID, conversationType, [=](/zim-harmonyos/guides/messaging/const-std::string-&conversationid,-zimconversationtype-conversationtype,-const-std::vector<std::shared_ptr<zimmessage>>-&messagelist,-const-zimerror-&errorinfo) {
