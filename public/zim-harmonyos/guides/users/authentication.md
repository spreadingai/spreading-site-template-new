...
收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_harmony~class~ZIM#renew-token) 接口更新 Token。
...
| 方法 | 描述 |
|-------|--------|
| [login](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_harmony~class~ZIM#login-2) | 登录 ZIM 服务。  |
| [renewToken](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_harmony~class~ZIM#renew-token) | 更新鉴权 Token。 |
| [tokenWillExpire](https://doc-zh.zego.im/article/api?doc=zim_API~javascript_harmony~interface~ZIMEventHandler#token-will-expire) | Token 过期回调。 |
