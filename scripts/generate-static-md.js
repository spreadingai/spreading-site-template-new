...
class StaticMDGenerator {
  ...
  generateSlugsForInstance(instance) {
    ...
    for (const sidebarKey in sidebars) {
      ...
      this.traverseSidebarItems(sidebarItems, instance.id, '', sidebarKey, [routeBasePath].filter(Boolean), slugs, instance.path);
    }
   ...
   * 遍历sidebar项目
   */
  traverseSidebarItems(items, instanceID, slugVersion, sidebarId, preSlug, result, instancePath) {
    if (!Array.isArray(items)) {
      return;
    }

    for (const item of items) {
      // 处理 autogenerated 类型：扫描目录并展开
      if (item.type === 'autogenerated' && item.dirName) {
        const expandedItems = this.expandAutogeneratedDir(instancePath, item.dirName);
        // 递归处理展开后的项目
        this.traverseSidebarItems(expandedItems, instanceID, slugVersion, sidebarId, preSlug, result, instancePath);
        continue;
      }

      if (item.items) {
        // 递归处理子项目
        this.traverseSidebarItems(item.items, instanceID, slugVersion, sidebarId, preSlug, result, instancePath);
      }
  ...
  /**
   * 展开 autogenerated 目录，扫描目录下的 .md/.mdx 文件
   */
  expandAutogeneratedDir(instancePath, dirName) {
    const IGNORE_FILE_NAME = ['.DS_Store'];
    const IGNORE_FOLDER_NAME = ['snippets', 'middleware'];

    const dirPath = path.join(this.ENTITY_ROOT_DIRECTORY, instancePath, dirName);

    const loop = (currentPath, relativeDir = '') => {
      ...
      // 排序：带数字前缀的在前
      const sortedFiles = this.sortFilesForAutogenerated(files);

      for (const file of sortedFiles) {
        if (IGNORE_FILE_NAME.includes(file) || IGNORE_FOLDER_NAME.includes(file)) {
          continue;
        }

        const filePath = path.join(currentPath, file);
        const childStats = fs.statSync(filePath);

        if (childStats.isDirectory()) {
          // 递归处理子目录
          const newRelativeDir = relativeDir ? path.join(relativeDir, file) : file;
          const childItems = loop(filePath, newRelativeDir);
          if (childItems.length > 0) {
            items.push({
              type: 'category',
              label: this.ignoreNumberPrefix(file),
              items: childItems
            });
          }
        } else {
          ...
          if (ext === '.mdx' || ext === '.md') {
            // 构建 id：dirName + 相对目录 + 文件名（不含扩展名）
            const idPath = relativeDir ? path.join(relativeDir, parsedPath.name) : parsedPath.name;
            const id = path.join(dirName, idPath).replace(/\\/g, '/');
            items.push({
              type: 'doc',
              id: this.ignoreNumberPrefix(id),
              label: this.ignoreNumberPrefix(parsedPath.name)
            });
          }
        }
      }
   ...
   * 排序文件：带数字前缀的在前
   */
  sortFilesForAutogenerated(files) {
    const numberFiles = files.filter(item => this.SEQUENCE_PREFIX_REGEX.test(item));
    const otherFiles = files.filter(item => !this.SEQUENCE_PREFIX_REGEX.test(item));
