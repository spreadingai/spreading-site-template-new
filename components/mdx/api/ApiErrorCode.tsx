    ...
    <ApiSection title={title}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {items.map((x, i) => (
            <tr key={i}>
              <td style={{ padding: 6, borderTop: "var(--docuo-table-border)" }}>{x.code}</td>
              <td style={{ padding: 6, borderTop: "var(--docuo-table-border)" }}>{x.name}</td>
              <td style={{ padding: 6, borderTop: "var(--docuo-table-border)" }}>{x.desc}</td>
            </tr>
          ))}
        </tbody>
      </table>
