...
export default function ApiMethod(props: ApiMethodProps) {
  ...
  return (
    <div id={anchorId} style={{ border: "var(--docuo-table-border)", borderRadius: 8, margin: "16px 0" }}>
      ...
      <div style={{ padding: "12px 16px", borderTop: "var(--docuo-table-border)", display: collapsed ? "none" : "block" }}>
        {belongClass && (
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {params.map((p) => (
                  <tr key={p.name}>
                    <td style={{ padding: "6px", borderTop: "var(--docuo-table-border)" }}>{p.name}</td>
                    <td style={{ padding: "6px", borderTop: "var(--docuo-table-border)", ...codeStyle }}>
                      {p.typeRef ? <a href={buildHref(p.typeRef)}>{p.type || "type"}</a> : p.type}
                    </td>
                    <td style={{ padding: "6px", borderTop: "var(--docuo-table-border)" }}>{p.desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
