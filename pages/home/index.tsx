...
export const getStaticProps = async ({ params }) => {
  console.log(new Date().toISOString().slice(0, 23), '[Spreading] Home getStaticProps:', params)
...
export default function Index(props) {
  ...
  if (typeof window !== "undefined") {
    ...
    if (isPreview !== undefined || treeData.length === 0) {
      const url = isPreview === 'true' ? `${router.basePath}/api/tree?isPreview=true` : `${router.basePath}/api/tree?isPreview=false`
