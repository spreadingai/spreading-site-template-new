...
<NpmBadge package="@Novel.sh/plugin-medium-zoom" />

将 [medium-zoom](https://github.com/francoischalifour/medium-zoom#readme) 集成到 Novel.sh 中，为图片提供可缩放的功能。
...
```bash
npm i -D @Novel.sh/plugin-medium-zoom@next
```

```ts
import { mediumZoomPlugin } from '@Novel.sh/plugin-medium-zoom'
...
@[code css](@Novel.sh/plugin-medium-zoom/src/client/styles/vars.css)
...
import { useMediumZoom } from '@Novel.sh/plugin-medium-zoom/client'
