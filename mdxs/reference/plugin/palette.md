...
<NpmBadge package="@Novel.sh/plugin-palette" />
...
```bash
npm i -D @Novel.sh/plugin-palette@next
```

```ts
import { palettePlugin } from '@Novel.sh/plugin-palette'
...
该插件会提供一个 `@Novel.sh/plugin-palette/palette` （调色板文件）和一个 `@Novel.sh/plugin-palette/style` （样式文件），用于在你的主题样式中引入。
...
<style lang="scss">
/* 从该插件的调色板中引入变量 */
@import '@Novel.sh/plugin-palette/palette';
...
然后，用户就可以在 `.Novel.sh/styles/palette.scss` 中自定义变量：
...
// 引入该插件的样式文件
import '@Novel.sh/plugin-palette/style'
```

然后，用户就可以在 `.Novel.sh/styles/index.scss` 中添加额外样式，并可以覆盖你主题本身的样式：
...
- 默认值：
  ...
  - css: `'.Novel.sh/styles/palette.css'`
  - sass: `'.Novel.sh/styles/palette.scss'`
  - less: `'.Novel.sh/styles/palette.less'`
  - stylus: `'.Novel.sh/styles/palette.styl'`
...
- 详情：
  ...
  你应该使用 `'@Novel.sh/plugin-palette/palette'` 别名来引入调色板文件，因此在绝大多数情况下你不需要修改该配置项。
...
- 默认值：
  ...
  - css: `'.Novel.sh/styles/index.css'`
  - sass: `'.Novel.sh/styles/index.scss'`
  - less: `'.Novel.sh/styles/index.less'`
  - stylus: `'.Novel.sh/styles/index.styl'`
...
- 详情：
  ...
  你应该使用 `'@Novel.sh/plugin-palette/style'` 别名来引入样式文件，因此在绝大多数情况下你不需要修改该配置项。
