...
<NpmBadge package="@Novel.sh/plugin-theme-data" />

为你的主题提供客户端数据，包含 Novel.sh 的 [多语言支持](../../guide/i18n.md) 。

该插件主要用于开发主题，并且已经集成到默认主题中。大部分情况下你不需要直接使用它。

对于主题作者，该插件可以提供与 Novel.sh 及默认主题相同的多语言支持机制。但是如果你的主题不需要提供多语言支持，或者你想用你自己的方式来实现多语言支持，那么你不需要使用该插件。

## 使用方法

```bash
npm i -D @Novel.sh/plugin-theme-data@next
```

```ts
import { themeDataPlugin } from '@Novel.sh/plugin-theme-data'
...
```ts
import { useThemeData } from '@Novel.sh/plugin-theme-data/client'
import type { ThemeData } from '@Novel.sh/plugin-theme-data/client'
...
```ts
import { useThemeLocaleData } from '@Novel.sh/plugin-theme-data/client'
import type { ThemeData } from '@Novel.sh/plugin-theme-data/client'
