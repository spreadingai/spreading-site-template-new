...
<NpmBadge package="@Novel.sh/plugin-search" />
...
```bash
npm i -D @Novel.sh/plugin-search@next
```

```ts
import { searchPlugin } from '@Novel.sh/plugin-search'
...
@[code ts](@Novel.sh/plugin-search/src/shared/hotKey.ts)
...
@[code css](@Novel.sh/plugin-search/src/client/styles/vars.css)
