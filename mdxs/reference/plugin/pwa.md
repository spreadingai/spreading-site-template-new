...
<NpmBadge package="@Novel.sh/plugin-pwa" />

使你的 Novel.sh 站点成为一个 [渐进式 Web 应用 (PWA)](https://developer.mozilla.org/zh-CN/docs/Web/Progressive_web_apps).
...
```bash
npm i -D @Novel.sh/plugin-pwa@next
```

```ts
import { pwaPlugin } from '@Novel.sh/plugin-pwa'
...
你需要将你的 Manifest 文件和图标放置在 [Public 目录](../../guide/assets.md#public-文件) 下。在下述的示例中，我们假设你正在使用默认的 Public 目录 `.Novel.sh/public` 。

1. 创建 Manifest 文件

通常是 `.Novel.sh/public/manifest.webmanifest` ：
...
- `.Novel.sh/public/images/icons/android-chrome-192x192.png`
- `.Novel.sh/public/images/icons/android-chrome-384x384.png`
...
```ts
import { usePwaEvent } from '@Novel.sh/plugin-pwa/client'
...
```ts
import { usePwaEvent, useSkipWaiting } from '@Novel.sh/plugin-pwa/client'
