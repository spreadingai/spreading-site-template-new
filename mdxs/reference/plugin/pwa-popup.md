...
<NpmBadge package="@Novel.sh/plugin-pwa-popup" />
...
```bash
npm i -D @Novel.sh/plugin-pwa-popup@next
```

```ts
import { pwaPlugin } from '@Novel.sh/plugin-pwa'
import { pwaPopupPlugin } from '@Novel.sh/plugin-pwa-popup'
...
@[code css](@Novel.sh/plugin-pwa-popup/src/client/styles/vars.css)
