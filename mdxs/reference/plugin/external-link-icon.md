...
<NpmBadge package="@Novel.sh/plugin-external-link-icon" />
...
```bash
npm i -D @Novel.sh/plugin-external-link-icon@next
```

```ts
import { externalLinkIconPlugin } from '@Novel.sh/plugin-external-link-icon'
...
@[code css](@Novel.sh/plugin-external-link-icon/src/client/styles/vars.css)
