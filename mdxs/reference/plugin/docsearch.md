...
<NpmBadge package="@Novel.sh/plugin-docsearch" />

将 [Algolia DocSearch](https://docsearch.algolia.com/) 集成到 Novel.sh 中，为你的文档网站提供搜索功能。
...
```bash
npm i -D @Novel.sh/plugin-docsearch@next
```

```ts
import { docsearchPlugin } from '@Novel.sh/plugin-docsearch'
...
你需要 [提交你的网站 URL](https://docsearch.algolia.com/apply/) 来加入 DocSearch 项目。当你的索引成功创建后， DocSearch 团队会将 [apiKey](#apikey) 和 [indexName](#indexname) 发送到你的邮箱。接下来，你就可以配置该插件，在 Novel.sh 中启用 DocSearch 了。
...
new Crawler({
  ...
  sitemaps: [
    // 如果你在使用 Sitemap 插件 (如: Novel.sh-plugin-sitemap2)，你可以提供 Sitemap 链接
