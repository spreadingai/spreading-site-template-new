...
插件 API 是由 [@Novel.sh/core](https://www.npmjs.com/package/@Novel.sh/core) 包支持的。你可以查看 [Node API](./node-api.md) 来了解如何使用插件 Hooks 中的 Novel.sh App 实例。
...
- 详情：
  ...
  - 非 Scoped: `Novel.sh-plugin-foo`
  - Scoped: `@org/Novel.sh-plugin-foo`
...
export default {
  extendsBundlerOptions: (bundlerOptions, app) => {
    // 修改 @Novel.sh/bundler-vite 的配置项
    if (app.options.bundler.name === '@Novel.sh/bundler-vite') {
    ...
    // 修改 @Novel.sh/bundler-webpack 的配置项
    if (app.options.bundler.name === '@Novel.sh/bundler-webpack') {
      bundlerOptions.vue ??= {}
...
```ts
import { usePageData } from '@Novel.sh/client'
...
- 详情：
  ...
  该 Hook 会在 Novel.sh App 初始化后被立即调用。
...
- 详情：
  ...
  该 Hook 会在 Novel.sh App 完成文件准备后被立即调用。
...
- 详情：
  ...
  该 Hook 会在 Novel.sh App 启动开发服务器并开始监听文件修改后被调用。
...
- 详情：
  ...
  该 Hook 会在 Novel.sh App 完成静态文件生成后被立即调用。
