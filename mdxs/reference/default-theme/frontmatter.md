...
- 详情：
  ...
  由[@Novel.sh/plugin-external-link-icon](../plugin/external-link-icon.md#externallinkicon) 提供。
...
然后你可以在 `.Novel.sh/styles/index.scss` 文件中为这个页面添加自定义样式：
...
features:
  ...
  - title: 高性能
    details: Novel.sh 会为每个页面预渲染生成静态的 HTML，同时，每个页面被加载的时候，将作为 SPA 运行。
---
```
