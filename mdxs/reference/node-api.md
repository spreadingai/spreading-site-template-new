...
Node API 是由 [@Novel.sh/core](https://www.npmjs.com/package/@Novel.sh/core) 包提供的。它是 [Novel.sh](https://www.npmjs.com/package/Novel.sh) 包的依赖之一，当然你也可以单独安装它：

```bash
npm i -D @Novel.sh/core@next
```
...
| 参数   | 类型        | 描述                       |
| ------ | ----------- | -------------------------- |
| config | `AppConfig` | 创建 Novel.sh App 的选项。 |
...
- 详情：
  ...
  Novel.sh App 的版本，即 `@Novel.sh/core` 包的版本。
...
- 工具函数：
  ...
  - `dir.source()`： 解析至源文件目录
  - `dir.dest()`： 解析至输出目录
  - `dir.client()`： 解析至 `@Novel.sh/client` 目录
...
| 参数    | 类型          | 描述                        |
| ------- | ------------- | --------------------------- |
| app     | `App`         | Novel.sh App 实例。         |
| options | `PageOptions` | 创建 Novel.sh Page 的选项。 |

- 详情：

  创建一个 Novel.sh Page 对象。

- 示例：

```ts
import { createPage } from '@Novel.sh/core'
