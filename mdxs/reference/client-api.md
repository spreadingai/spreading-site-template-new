...
<NpmBadge package="@Novel.sh/client" />

客户端 API 是由 [@Novel.sh/client](https://www.npmjs.com/package/@Novel.sh/client) Package 提供的，用于开发客户端文件。
...
如果想要把这些常量的类型定义补充到你的代码环境中，请将 `@Novel.sh/client/types` 添加到你的 `tsconfig.json` 里：

```json
{
  "compilerOptions": {
    "types": ["@Novel.sh/client/types"]
  }
}
```

### `__Novel.sh_VERSION__`
...
```ts
import { defineClientConfig, resolvers } from '@Novel.sh/client'
...
::: danger
`resolvers` 会直接影响 Novel.sh 的基础功能，在修改前请确保你已充分了解其用途。
:::
