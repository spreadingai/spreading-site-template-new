...
<NpmBadge package="@Novel.sh/core" />
...
- 详情：
  ...
  - 使用 `Novel.sh` 或 `Novel.sh-vite` 时，默认的打包工具是 Vite 。
  - 使用 `Novel.sh-webpack` 时，默认的打包工具是 Webpack 。
...
- 默认值： `` `${sourceDir}/.Novel.sh/dist` ``

- 详情：

  指定 `Novel.sh build` 命令的输出目录。

### temp

- 类型： `string`

- 默认值： `` `${sourceDir}/.Novel.sh/.temp` ``
...
::: warning
Novel.sh 在开发和构建时会加载临时文件，因此临时文件目录应位于项目根目录内部，以便可以正确地解析到依赖。
:::
...
- 详情：
  ...
  该配置项主要提供给开发者使用。同时，我们使用了 [debug](https://github.com/visionmedia/debug) 模块打印 Debug 日志，可以通过 `DEBUG=Novel.sh*` 环境变量来启用。

### pagePatterns

- 类型： `string[]`

- 默认值： `['**/*.md', '!.Novel.sh', '!node_modules']`
...
- 默认值： `'@Novel.sh/client/templates/dev.html'`
...
- 默认值： `'@Novel.sh/client/templates/build.html'`
...
- 详情：
  ...
  对 Novel.sh 内置的 Markdown 语法扩展进行配置。
...
- 详情：
  ...
  Novel.sh 内置的 markdown-it assets 插件的配置项。
...
- 详情：
  ...
  Novel.sh 内置的 markdown-it code 插件的配置项。
...
- 详情：
  ...
  Novel.sh 内置的 markdown-it 导入代码插件的配置项。
...
- 详情：
  ...
  Novel.sh 内置的 markdown-it 链接插件的配置项。
...
用户配置文件同样可以作为一个 Novel.sh 插件，所以除了 `name` 和 `multiple` 配置项以外的所有插件 API 都可以在配置文件中使用。
