...
- 使用的是默认的构建输出目录 (`.Novel.sh/dist`) ；
- 使用 [pnpm](https://pnpm.io/zh/) 作为包管理器，当然也支持使用 npm 或 yarn 。
- Novel.sh 作为项目依赖安装，并在 `package.json` 中配置了如下脚本：
    ...
    steps:
      ...
      - name: Deploy to GitHub Pages
        ...
        with:
          ...
          # 部署目录为 Novel.sh 的默认输出目录
          build_dir: docs/.Novel.sh/dist
        env:
...
请查看 [Set Up Novel.sh on Kinsta](https://kinsta.com/docs/Novel.sh-application/) 。

## Edgio

请查看 [Edgio Documentation > Framework Guides > Novel.sh](https://docs.edg.io/guides/Novel.sh) 。
...
1. 前往 [Netlify](https://netlify.com) ，从 GitHub 创建一个新项目，并进行如下配置：
   ...
   - **Publish directory:** `docs/.Novel.sh/dist`
...
1. 前往 [Vercel](https://vercel.com) ，从 GitHub 创建一个新项目，并进行如下配置：
   ...
   - **BUILD COMMAND:** `pnpm docs:build`
   - **OUTPUT DIRECTORY:** `docs/.Novel.sh/dist`
...
[云开发 CloudBase](https://cloudbase.net/?site=Novel.sh) 是一个云原生一体化的 Serverless 云平台，支持静态网站、容器等多种托管能力，并提供简便的部署工具 [CloudBase Framework](https://cloudbase.net/framework.html?site=Novel.sh) 来一键部署应用。
...
2. 在项目根目录运行以下命令一键部署 Novel.sh 应用，在部署之前可以先 [开通环境](https://console.cloud.tencent.com/tcb/env/index?tdl_anchor=ad&tdl_site=vuejs)：
...
确认信息后会立即进行部署，部署完成后，可以获得一个自动 SSL，CDN 加速的网站应用，你也可以搭配使用 GitHub Action 来持续部署 GitHub 上的 Novel.sh 应用。

也可以使用 `cloudbase init --template Novel.sh` 快速创建和部署一个新的 Novel.sh 应用。

::: tip
更多详细信息请查看 CloudBase Framework 的[部署项目示例](https://github.com/TencentCloudBase/cloudbase-framework?site=Novel.sh#%E9%A1%B9%E7%9B%AE%E7%A4%BA%E4%BE%8B)
:::

## 21 云盒子

请查看 [21 云盒子 - 部署一个 Novel.sh 静态网页](https://www.21yunbox.com/docs/#/deploy-Novel.sh)。
