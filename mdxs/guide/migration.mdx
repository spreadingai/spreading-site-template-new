...
::: warning
Novel.sh v1 的插件和主题与 Novel.sh v2 不兼容。你需要将它们升级到与 v2 对应的版本。
:::

Novel.sh v2 的一些主要改动和优化：

- Novel.sh v2 现在使用 Vue 3 ，因此你要保证你的组件和其他客户端文件是适用于 Vue 3 的。
- Novel.sh v2 是使用 TypeScript 开发的，因此它现在提供了更好的类型支持。我们强烈推荐你使用 TypeScript 来开发插件和主题。 Novel.sh 配置文件也同样支持 TypeScript ，你可以直接使用 `.Novel.sh/config.ts` 。
- Novel.sh v2 支持使用 Webpack 和 Vite 作为打包工具。现在默认的打包工具是 Vite ，但你仍然可以选择使用 Webpack 。你甚至可以在开发模式使用 Vite 来获取更好的开发体验，而在构建模式使用 Webpack 来获取更好的浏览器兼容性。
- Novel.sh v2 现在是纯 ESM 包， CommonJS 格式的配置文件不再被支持。

Novel.sh v2 的核心思想和流程是和 v1 一致的，但 v2 API 经过了重新设计，更加标准化。因此在将现有的 v1 项目迁移至 v2 时，你很可能会遇到一些 Breaking Changes 。本指南将帮助你将 v1 的站点 / 插件 / 主题迁移至 v2 。
...
```diff
- module.exports = {
-   theme: '@Novel.sh/theme-default',
-   themeConfig: {
-     // 默认主题配置
-   },
- }

+ import { defaultTheme } from 'Novel.sh'
+ export default {
+   theme: defaultTheme({
+     // 默认主题配置
+   })
...
-   plugins: [
-     [
-       '@Novel.sh/plugin-google-analytics',
-       {
-         id: 'G-XXXXXXXXXX',
-       },
-     ],
...
所有 Webpack 相关的配置都移动至 `@Novel.sh/bundler-webpack` 的配置项中，包括：
...
Novel.sh v1 的 Stylus 调色板系统 （即 `styles/palette.styl` 和 `styles/index.styl`） 不再由 Novel.sh Core 默认提供支持。

调色板系统提取到了 [@Novel.sh/plugin-palette](../reference/plugin/palette.md) 当中。
...
#### .Novel.sh/enhanceApp.js

重命名为 `.Novel.sh/client.{js,ts}` ，使用方法也有改动。
...
你需要使用 [@Novel.sh/plugin-register-components](../reference/plugin/register-components.md) ，或者在 `.Novel.sh/client.{js,ts}` 中手动注册你的组件。
...
- Webpack 相关的 Hook 都被移除了，因为 Novel.sh Core 已经和 Webpack 解耦了。你可以尝试使用 [extendsBundlerOptions](../reference/plugin-api.md#extendsbundleroptions) Hook 来进行相似的操作，但要注意应适配所有不同的打包工具。
...
- 所谓的 **主题目录结构约定** 不再存在。
  ...
  - `theme/global-components/` 目录下的文件不会被自动注册为 Vue 组件。你需要使用 [@Novel.sh/plugin-register-components](../reference/plugin/register-components.md) ，或者在 `clientConfigFile` 中手动注册组件。
...
- `themeConfig` 已经从用户配置和站点数据中移除。如果你想要像 v1 一样通过 `this.$site.themeConfig` 来访问 `themeConfig` ，我们现在建议使用 [@Novel.sh/plugin-theme-data](../reference/plugin/theme-data.md) 插件和它提供的 Composition API `useThemeData` 。
- Stylus 不再是默认的 CSS 预处理器，并且 Stylus 调色板系统不再被默认支持。如果你仍然想要使用和 v1 类似的调色板系统，可以使用 [@Novel.sh/plugin-palette](../reference/plugin/palette.md) 。
- 由 Prism.js 提供的 Markdown 代码块的语法高亮不再被默认支持。你可以选择使用 [@Novel.sh/plugin-prismjs](../reference/plugin/prismjs.md) 或 [@Novel.sh/plugin-shiki](../reference/plugin/shiki.md) ，或者用你自己的方式实现语法高亮。
