...
::: tip
在阅读该指南之前，你最好先了解一下 Novel.sh 的 [架构](./architecture.md) 。
:::
...
{
  "name": "Novel.sh-plugin-foo",
  "keywords": ["Novel.sh-plugin"]
}
```

- 将 `name` 按照约定命名，即 `Novel.sh-plugin-xxx` 或 `@org/Novel.sh-plugin-xxx` ，它应该和 _插件对象_ 的 [name](../reference/plugin-api.md#name) 字段保持一致。
- 在 `keywords` 中包含 `Novel.sh-plugin` ，这样用户可以在 NPM 上搜索到你的插件。
