...
const render = mustache_1.default.render;
...
function pluginOpenAPIDocs(context, options) {
  ...
  async function generateApiDocs(options, pluginId) {
    var _a, _b, _c;
    console.log(
      chalk_1.default.green(`###### generateApiDocs pluginId "${pluginId}"`)
    );
    console.log(
      chalk_1.default.green(
        `###### generateApiDocs options "${JSON.stringify(options)}"`
      )
    );
    let {
      specPath,
    ...
    try {
      ...
      const [loadedApi, tags] = await (0, openapi_1.processOpenapiFiles)(
        openapiFiles,
        options,
        sidebarOptions
      );
      if (!fs_1.default.existsSync(outputDir)) {
        try {
          ...
          console.log(
            chalk_1.default.green(`Successfully created "${outputDir}"`)
          );
        } catch (err) {
          console.error(
            chalk_1.default.red(`Failed to create "${outputDir}"`),
            chalk_1.default.yellow(err)
          );
        }
      }
      ...
      if (
        Object.keys(
          sidebarOptions !== null && sidebarOptions !== void 0
            ? sidebarOptions
            : {}
        ).length > 0
      ) {
        const sidebarSlice = (0, sidebars_1.default)(
          sidebarOptions,
          options,
          loadedApi,
          tags,
        ...
        if (!fs_1.default.existsSync(`${outputDir}/sidebar.js`)) {
          try {
            ...
            console.log(
              chalk_1.default.green(
                `Successfully created "${outputDir}/sidebar.js"`
              )
            );
          } catch (err) {
            console.error(
              chalk_1.default.red(`Failed to write "${outputDir}/sidebar.js"`),
              chalk_1.default.yellow(err)
            );
          }
        }
...
\`\`\`
      ...
      const apiPageGenerator =
        (_a =
          markdownGenerators === null || markdownGenerators === void 0
            ? void 0
            : markdownGenerators.createApiPageMD) !== null && _a !== void 0
          ? _a
          : markdown_1.createApiPageMD;
      const infoPageGenerator =
        (_b =
          markdownGenerators === null || markdownGenerators === void 0
            ? void 0
            : markdownGenerators.createInfoPageMD) !== null && _b !== void 0
          ? _b
          : markdown_1.createInfoPageMD;
      const tagPageGenerator =
        (_c =
          markdownGenerators === null || markdownGenerators === void 0
            ? void 0
            : markdownGenerators.createTagPageMD) !== null && _c !== void 0
          ? _c
          : markdown_1.createTagPageMD;
        ...
        if (item.type === "api") {
          if (!fs_1.default.existsSync(`${outputDir}/${item.id}.api.mdx`)) {
            try {
              ...
              console.error(
                chalk_1.default.red(
                  `Failed to write "${outputDir}/${item.id}.api.mdx"`
                ),
                chalk_1.default.yellow(err)
              );
            }
        ...
        if (item.type === "info") {
          if (!fs_1.default.existsSync(`${outputDir}/${item.id}.info.mdx`)) {
            try {
              ...
              console.error(
                chalk_1.default.red(
                  `Failed to write "${outputDir}/${item.id}.info.mdx"`
                ),
                chalk_1.default.yellow(err)
              );
            }
        ...
        if (item.type === "tag") {
          if (!fs_1.default.existsSync(`${outputDir}/${item.id}.tag.mdx`)) {
            try {
              ...
              console.error(
                chalk_1.default.red(
                  `Failed to write "${outputDir}/${item.id}.tag.mdx"`
                ),
                chalk_1.default.yellow(err)
              );
            }
  ...
  async function cleanApiDocs(options) {
    ...
    const apiDir = (0, utils_1.posixPath)(
      path_1.default.join(siteDir, outputDir)
    );
    const apiMdxFiles = await (0, utils_1.Globby)(
    ...
    apiMdxFiles.map((mdx) =>
      fs_1.default.unlink(`${apiDir}/${mdx}`, (err) => {
        if (err) {
          console.error(
            chalk_1.default.red(`Cleanup failed for "${apiDir}/${mdx}"`),
            chalk_1.default.yellow(err)
          );
        } else {
          console.log(
            chalk_1.default.green(`Cleanup succeeded for "${apiDir}/${mdx}"`)
          );
        }
      })
    );
    sidebarFile.map((sidebar) =>
      fs_1.default.unlink(`${apiDir}/${sidebar}`, (err) => {
        if (err) {
          console.error(
            chalk_1.default.red(`Cleanup failed for "${apiDir}/${sidebar}"`),
            chalk_1.default.yellow(err)
          );
        } else {
          console.log(
            chalk_1.default.green(
              `Cleanup succeeded for "${apiDir}/${sidebar}"`
            )
          );
        }
      })
  ...
  async function generateVersions(versions, outputDir) {
    ...
    try {
      ...
      console.log(
        chalk_1.default.green(
          `Successfully created "${outputDir}/versions.json"`
        )
      );
    } catch (err) {
      console.error(
        chalk_1.default.red(`Failed to write "${outputDir}/versions.json"`),
        chalk_1.default.yellow(err)
      );
    }
  }
    ...
    if (fs_1.default.existsSync(`${outputDir}/versions.json`)) {
      fs_1.default.unlink(`${outputDir}/versions.json`, (err) => {
        if (err) {
          console.error(
            chalk_1.default.red(
              `Cleanup failed for "${outputDir}/versions.json"`
            ),
            chalk_1.default.yellow(err)
          );
        } else {
          console.log(
            chalk_1.default.green(
              `Cleanup succeeded for "${outputDir}/versions.json"`
            )
          );
        }
      });
        ...
        async (args, instance) => {
          ...
          if (pluginId) {
            try {
              ...
              targetConfig =
                (_a = pluginConfig.config) !== null && _a !== void 0 ? _a : {};
          ...
          if (id === "all") {
            ...
            console.error(
              chalk_1.default.red(
                `ID '${id}' does not exist in OpenAPI docs config.`
              )
            );
          } else {
